...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	if slideExist or button.ask or button.slidename then
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		drawSprite(parentHeader, childHeader, self.Width - spriteW / 2, self.Height - intH / 2, spriteW, intH, .0, self.Base.HeaderColor[7], self.Base.HeaderColor[8], self.Base.HeaderColor[9], 215)
		self.Height = self.Height - 0.03
		if not self.Base.CustomHeader then
			DrawText2(1, self.Base.Title, .8, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white, false, 0)
		end
	end
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		local intH, scale = 0.0275, 0.290
		self.Height = self.Height - 0.04
		-- DrawRect(self.Width - _intW / 2, self.Height - 0.007, _intW, 0.0025, 0, 0, 0, 255)
		-- DrawRect(self.Width - _intW / 2, self.Height + 0.0302, _intW, 0.0025, 0, 0, 0, 255)
		...
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - -0.017, _intW, nwintH, .0, 255, 255, 255, 255)
		DrawText2(0, descText, scale, self.Width - _intW + .005, self.Height - -0.017 + 0.003, color_white)
	end
	local arrows = self.Base.arrowsonly or self.Menu[self.Data.currentMenu] and self.Menu[self.Data.currentMenu].arrowsonly or tableButtons[self.Pag[3]] and tableButtons[self.Pag[3]].arrowsonly
	if arrows and not hasHelp then
		local intH, scale = 0.0275, 0.260
		self.Height = self.Height - 0.003
		self.Height = self.Height + intH / 2
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + intH / 1.9 - 0.020, _intW, 0.035, .0, 255, 255, 255, 255)
		DrawSprite("commonmenu", "shop_arrows_upanddown", self.Width - _intW / 2, self.Height + intH / 1.9 - 0.020, 0.027, 0.04, .0, 255, 255, 255, 255)
	end
end
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		if self.Base.world then 
			local ScaleformMovie = RequestScaleformMovie("MP_MENU_GLARE")
			while not HasScaleformMovieLoaded(ScaleformMovie) do
				Citizen.Wait(0)
			end
			DrawScaleformMovie(ScaleformMovie,  self.Width+ 0.163, self.Height + 0.295, spriteW + 0.5, intH + 0.733, 255, 255, 255, 255, 0)
		end

		self.Height = self.Height - 0.03
		if not self.Base.CustomHeader and headername ~= nil then
			DrawText2(headername.var, headername.header, .7, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white, false, 0)
		elseif self.Base.Title and headername == nil then 
	...
	self.Height = self.Height + 0.06
	local rectW, rectH = _intW, _intH - .005
	if not currentMenu.mother then
		DrawRect(self.Width - rectW / 2, self.Height - rectH / 2, rectW, rectH, self.Base.Color[1], self.Base.Color[2], self.Base.Color[3], 255)
	end
	self.Height = self.Height + 0.005
	if not currentMenu.mother then
	...
	self.Height = self.Height + 0.005
	DrawText2(0, stringCounter, .275, self.Width - rectW / 2 + .11, self.Height - _intH, color_white, true, 2)  
	if currentMenu and currentMenu.charCreator and currentMenu.mother ~= nil then
		local spriteW, spriteH = .264, .21
		self.Height = self.Height + spriteH - 0.01
		if currentMenu.mother then
			spriteW, spriteH = .025, .24
			local customX = self.Width - .1
			drawSprite("root_cause", currentMenu.mother, customX - spriteW / 2, self.Height - spriteH / 2, spriteW + 0.2, spriteH + 0.0028, 0.0, 255, 255, 255, 255)
			self.Height = self.Height + 0.01
		end
	end
	self.Height = self.Height + 0.005
...
------------------------ KKI STOCK, STOCK TU SERAS PLUS LEGER 